...
function Get-GraphUserList{
    ...
    if ($Name)     {
        $PSBoundParameters['Filter'] = "`$filter=startswith(displayName,'{0}') or startswith(givenName,'{0}') or startswith(surname,'{0}') or startswith(mail,'{0}') or startswith(userPrincipalName,'{0}')" -f $Name
        $PSBoundParameters.Remove('Name')
    }
    Write-Progress "Getting the List of users"
    Get-MgUser @PSBoundParameters -All
    # "GraphUser" type has changed to Microsoft.Graph.PowerShell.Models.MicrosoftGraphUser1
