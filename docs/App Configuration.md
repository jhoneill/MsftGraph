...
The Microsoft.Graph.Authentication has an alias `Connect-Graph` and calling this will call `Connect-MgGraph` 
Microsoft.Graph.PlusPlus replaces this with a `Connect-Graph` function with extra functionality, it still calls `Connect-MgGraph`and if no other parameters are provided `Connect-MgGraph` will logs a user on using the **Device-Code method** . This method means the app tells the login service "I'm am the app with ID '14d82eec-204b-4c2f-b7e8-296a70dab67e' and **I want to use the these scopes** of functionality. If I send a user to you, can you give me a token when they logon?" The service responds "Tell the user to do these steps, which will identify our conversation." The app passes the instructions to the user, and polls the server until it gets a token or a failure message. 

We can **specify a set of scopes **with `Set-GraphConnectionOptions -DefaultScopes`. So you don't need to do this every time,  Microsoft.Graph.PlusPlus runs that script, which contains a list of scopes. The script will check for an environment environment variable `GraphScopes` for a comma seperated list of scopes, and if it doesn't find one it will use it's own list. The default script is named `AuthSettings.ps1` in the module directory. If you don't like changing files in the module directory you can make a copy and point to it with the environment variable `MGSettingsPath` If the default is not set or passed as a parameter `Connect-MgGraph` assumes a basic set.    
...
`Connect-MgGraph` provides **"Bring your own token"** support so can we skip the Device-logon process. `Connect-Graph` uses this to provide **three additional ways to obtain a token, without a dialog between user and logon service**. You can set a custom App ID for a username and password logon by calling `Set-GraphConnectionOptions -ClientID` in the same script which sets the scopes, but by default the script sets the client ID to the app ID of *Microsoft Graph PowerShell*. Removing the dialog means the logon service can't ask for consent for an app to use a set of scopes, so consent must have been given to the app beforehand.

As well as users being *security principals*, who consent to some scoped permission being *delegated* to apps, apps themselves can be security principals and receive consent to  act in their own right. If you provide your Tenant ID, the ID of an app registered in the tenant, and the app's secret, you can run `Connect-Graph -AsApp` to logon as the app. This is the first of the three extra logon methods: the *Microsoft Graph PowerShell* app isn't a security principal in your tenant, and your tenant doesn't have a secret (password) for it. So it can be used for username/password logon (which doesn't need a secret) - the second method but -AsApp logon requires a registered app. 

 You can set the tenant ID in the settings script with `Set-GraphConnectionOptions -TenantID` and the secret with "`Set-GraphConnectionOptions -TenantID` If TenantID and client ID are set `Connect-Graph` enables the `Credential` parameter (which takes a credential object ) and if the client secret is set it enables the `AsApp` switch parameter
