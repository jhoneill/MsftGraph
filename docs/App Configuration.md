...
We can **specify a set of scopes **with `Set-GraphOptions -DefaultScopes`. So you don't need to do this every time,  Microsoft.Graph.PlusPlus runs a settings script, which contains a list of scopes. The script will check for an environment environment variable `GraphScopes` for a comma seperated list of scopes, and if it doesn't find one it will use its own list. The default script is named `Microsoft.Graph.PlusPlus.settings.ps1` in the module directory and sets other options as well, so you can set scopes via environment variable or script as you choose. If you don't like changing files in the module directory you can make a copy and point to it with the environment variable `GraphSettingsPath`. If the default is not set or passed as a parameter nothing will be passed to `Connect-MgGraph`, which will assumes a basic set of scopes.    
